...
export class PlayerController {
  ...
  attachTouchControls(container: HTMLElement): void {
    ...
    controls.style.cssText = 'position:absolute;inset:0;display:none;pointer-events:none;z-index:12;';

    const joystick = document.createElement('div');
    joystick.style.cssText = 'position:absolute;left:22px;bottom:24px;width:132px;height:132px;border:2px solid rgba(255,255,255,0.5);border-radius:50%;background:rgba(20,43,27,0.48);pointer-events:all;touch-action:none;';
    const stick = document.createElement('div');
    stick.style.cssText = 'position:absolute;left:50%;top:50%;width:58px;height:58px;margin:-29px;border-radius:50%;background:rgba(255,227,109,0.92);border:3px solid #fff8c7;box-shadow:0 5px 14px rgba(0,0,0,0.3);pointer-events:none;';
    ...
    interact.style.cssText = 'position:absolute;right:22px;bottom:34px;width:112px;height:112px;border-radius:50%;border:3px solid #fff8c7;background:rgba(47,143,58,0.92);color:white;font:900 14px Segoe UI,sans-serif;letter-spacing:0;box-shadow:0 5px 14px rgba(0,0,0,0.3);pointer-events:all;touch-action:none;';
    ...
    const releaseInteract = (): void => { this.interactPressed = false; };
    ...
    const touchQuery = window.matchMedia('(pointer: coarse), (max-width: 900px)');
    const updateVisibility = (): void => {
      controls.style.display = touchQuery.matches || navigator.maxTouchPoints > 0 ? 'block' : 'none';
    };
    updateVisibility();
