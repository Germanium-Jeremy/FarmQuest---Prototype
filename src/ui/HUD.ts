...
const titleCase = (text: string) => text.replace(/\b\w/g, (l) => l.toUpperCase());
...
export class HUD {
  ...
  constructor(private overlay: HTMLElement, private scoreManager: ScoreManager) {
    ...
    this.hudEl = this.mkDiv('position:absolute;top:16px;left:50%;transform:translateX(-50%);width:min(94vw,620px);background:linear-gradient(135deg,rgba(20,43,27,0.94),rgba(32,65,42,0.9));color:white;border:2px solid rgba(255,232,126,0.42);border-radius:18px;padding:15px 18px;box-shadow:0 18px 42px rgba(0,0,0,0.26);display:none;pointer-events:none;overflow:hidden;');
    this.promptEl = this.mkDiv('position:absolute;bottom:72px;left:50%;transform:translateX(-50%);background:rgba(18,24,20,0.9);color:#ffe36d;padding:12px 22px;border-radius:999px;font-size:18px;font-weight:900;border:2px solid #ffe36d;display:none;pointer-events:none;box-shadow:0 12px 26px rgba(0,0,0,0.22);');
    this.feedbackEl = this.mkDiv('position:absolute;top:165px;left:50%;transform:translateX(-50%);width:min(90vw,420px);background:#fffdf2;color:#173320;border-left:8px solid #52a447;border-radius:14px;padding:13px 16px;font-size:18px;font-weight:900;display:none;box-shadow:0 14px 34px rgba(0,0,0,0.2);pointer-events:none;text-align:center;');
    this.taskModalEl = this.mkDiv('position:absolute;inset:0;display:none;justify-content:center;align-items:center;background:radial-gradient(circle at 50% 42%,rgba(63,117,67,0.28),rgba(8,13,10,0.58));pointer-events:all;color:white;text-align:center;padding:20px;z-index:20;');
    this.screenEl = this.mkDiv('position:absolute;inset:0;display:none;justify-content:center;align-items:center;background:rgba(12,19,15,0.86);pointer-events:all;color:white;text-align:center;padding:24px;z-index:20;');
  }

  private mkDiv(css: string): HTMLElement { const d = document.createElement('div'); d.style.cssText = css; this.overlay.appendChild(d); return d; }
  ...
  updateHUD(task: GameTask | null, timeRemaining: number, currentTaskIndex?: number, totalTasks?: number): void {
    ...
    const info = (currentTaskIndex != null && totalTasks != null) ? `TASK ${currentTaskIndex + 1} / ${totalTasks}` : '';
  ...
  showRegistration(onRegister: (email: string, displayName?: string) => void, errorMessage = '', loading = false): void {
    this.hideHUD(); this.hideTaskModal();
    ...
    document.getElementById('reg-form')!.addEventListener('submit', (e) => { e.preventDefault(); onRegister((document.getElementById('email-input') as HTMLInputElement).value, (document.getElementById('name-input') as HTMLInputElement).value); });
    for (const id of ['email-input', 'name-input']) { document.getElementById(id)?.addEventListener('keydown', (e) => e.stopPropagation()); document.getElementById(id)?.addEventListener('keyup', (e) => e.stopPropagation()); }
  }

  showLogin(onLogin: (email: string) => void, onRegister: (email: string, displayName: string) => void, errorMessage = '', loading = false): void {
    this.hideHUD(); this.hideTaskModal();
    this.screenEl.style.display = 'flex';
    ...
    document.getElementById('show-reg')?.addEventListener('click', (e) => { e.preventDefault(); (document.getElementById('login-mode') as HTMLElement).style.display = 'none'; (document.getElementById('reg-mode') as HTMLElement).style.display = 'block'; });
    document.getElementById('show-login')?.addEventListener('click', (e) => { e.preventDefault(); (document.getElementById('reg-mode') as HTMLElement).style.display = 'none'; (document.getElementById('login-mode') as HTMLElement).style.display = 'block'; });
    document.getElementById('login-btn')?.addEventListener('click', () => { const e = (document.getElementById('login-email') as HTMLInputElement).value; if (e) onLogin(e); });
    document.getElementById('reg-btn')?.addEventListener('click', () => { const e = (document.getElementById('reg-email') as HTMLInputElement).value; const n = (document.getElementById('reg-name') as HTMLInputElement).value; if (e) onRegister(e, n); });
  }

  showCharacterSelect(options: { type: string; label: string; icon: string }[], onSelect: (type: string) => void): void {
    this.hideHUD(); this.hideTaskModal();
    this.screenEl.style.display = 'flex';
    const cards = options.map((o) => `<div class="cc" data-t="${o.type}" style="cursor:pointer;padding:20px;border-radius:16px;border:3px solid transparent;background:rgba(255,255,255,0.12);transition:all 0.2s;min-width:120px;text-align:center;"><div style="font-size:48px;margin-bottom:8px;">${o.icon}</div><div style="font-size:18px;font-weight:900;">${o.label}</div></div>`).join('');
    ...
    els.forEach((c) => c.addEventListener('click', () => { els.forEach((x) => { (x as HTMLElement).style.borderColor = 'transparent'; (x as HTMLElement).style.background = 'rgba(255,255,255,0.12)'; }); (c as HTMLElement).style.borderColor = '#ffe36d'; (c as HTMLElement).style.background = 'rgba(255,227,109,0.15)'; sel = (c as HTMLElement).dataset.t ?? ''; }));
    const btn = document.createElement('button'); btn.textContent = 'Select Character'; btn.style.cssText = this.btn('#52a447'); btn.addEventListener('click', () => { if (sel) onSelect(sel); });
    this.screenEl.querySelector('div')?.appendChild(btn);
  }

  showMapSelect(options: { id: string; name: string; description: string; icon: string }[], onSelect: (id: string) => void): void {
    this.hideHUD(); this.hideTaskModal();
    this.screenEl.style.display = 'flex';
    const cards = options.map((o) => `<div class="mc" data-id="${o.id}" style="cursor:pointer;padding:20px;border-radius:16px;border:3px solid transparent;background:rgba(255,255,255,0.12);transition:all 0.2s;min-width:140px;text-align:center;"><div style="font-size:48px;margin-bottom:8px;">${o.icon}</div><div style="font-size:20px;font-weight:900;">${o.name}</div><div style="font-size:14px;color:#d8f5c6;margin-top:4px;">${o.description}</div></div>`).join('');
    ...
    els.forEach((c) => c.addEventListener('click', () => { els.forEach((x) => { (x as HTMLElement).style.borderColor = 'transparent'; (x as HTMLElement).style.background = 'rgba(255,255,255,0.12)'; }); (c as HTMLElement).style.borderColor = '#ffe36d'; (c as HTMLElement).style.background = 'rgba(255,227,109,0.15)'; sel = (c as HTMLElement).dataset.id ?? ''; }));
    const btn = document.createElement('button'); btn.textContent = 'Select Map'; btn.style.cssText = this.btn('#52a447'); btn.addEventListener('click', () => { if (sel) onSelect(sel); });
    this.screenEl.querySelector('div')?.appendChild(btn);
  }
  ...
  showLobby(playerCount: number, displayName: string, characterType: string, mapName: string): void {
    this.hideHUD(); this.hideTaskModal();
    this.screenEl.style.display = 'flex';
    const ci: Record<string, string> = { male: '👨', female: '👩', robot: '🤖' };
    const mi: Record<string, string> = { rwanda: '🌧️', sudan: '🏜️', seychelles: '🌊' };
  ...
  showLeaderboard(entries: { rank: number; displayName: string; score: number; completionTime: number }[], yourRank: number, yourScore: number, isTopTen: boolean, onPlayAgain: () => void): void {
    this.hideHUD(); this.hideTaskModal();
    this.screenEl.style.display = 'flex';
    const rows = entries.slice(0, 10).map((e) => { const m = e.rank === 1 ? '🥇' : e.rank === 2 ? '🥈' : e.rank === 3 ? '🥉' : ''; const h = e.rank === yourRank ? 'background:rgba(255,227,109,0.15);' : ''; const t = `${Math.floor(e.completionTime / 60)}:${String(Math.floor(e.completionTime % 60)).padStart(2, '0')}`; return `<div style="${h}padding:8px 12px;border-radius:8px;margin-bottom:4px;display:flex;justify-content:space-between;font-size:18px;"><span>${m} ${e.rank}. ${e.displayName}</span><span style="color:#ffe36d;">${e.score} pts (${t})</span></div>`; }).join('');
  ...
  showFirstTask(task: GameTask, onStart: () => void): void {
    ...
    this.taskModalEl.innerHTML = this.card({ eyebrow: 'Welcome to FarmQuest', kicker: 'Your First Task', icon: taskIcon(task), title: titleCase(task.description), body: `Complete the farming challenges before time runs out.<br>Progress ${task.currentAmount} / ${task.targetAmount}`, buttonText: 'Start Game' });
    document.getElementById('task-modal-button')!.addEventListener('click', onStart);
  }
  ...
  showTaskTransition(completedTask: GameTask, nextTask: GameTask | null, onDone: () => void): void {
    ...
    this.hideHUD();
    this.taskModalEl.style.display = 'flex';
    this.taskModalEl.innerHTML = this.card({ eyebrow: 'Task Complete!', icon: '✓', title: titleCase(completedTask.description), body: `Progress ${completedTask.targetAmount} / ${completedTask.targetAmount}. +${completedTask.scoreReward} points` });
    this.transitionTimer = window.setTimeout(() => {
      if (!nextTask) { onDone(); return; }
      this.taskModalEl.innerHTML = this.card({ eyebrow: 'Next Task', icon: taskIcon(nextTask), title: titleCase(nextTask.description), body: `Progress ${nextTask.currentAmount} / ${nextTask.targetAmount}` });
      this.transitionTimer = window.setTimeout(() => { this.hideTaskModal(); onDone(); }, 1500);
    }, 1150);
  }
  ...
  showPrompt(text: string): void { this.promptEl.style.display = 'block'; this.promptEl.textContent = `[E] ${text}`; }
  hidePrompt(): void { this.promptEl.style.display = 'none'; }
  showFeedback(message: string): void { this.feedbackEl.textContent = message; this.feedbackEl.style.display = 'block'; this.feedbackTimer = 1.8; }
  hideScreen(): void { this.screenEl.style.display = 'none'; }
  hideHUD(): void { this.hudEl.style.display = 'none'; this.promptEl.style.display = 'none'; }
  hideTaskModal(): void { if (this.transitionTimer) window.clearTimeout(this.transitionTimer); this.transitionTimer = null; this.taskModalEl.style.display = 'none'; }

  update(delta: number): void { if (this.feedbackTimer <= 0) return; this.feedbackTimer -= delta; if (this.feedbackTimer <= 0) this.feedbackEl.style.display = 'none'; }

  private card(c: { eyebrow: string; icon: string; title: string; body: string; kicker?: string; buttonText?: string }): string {
  ...
  private btn(color: string): string { return `padding:15px 36px;font-size:20px;font-weight:1000;background:${color};color:white;border:0;border-radius:999px;cursor:pointer;pointer-events:all;box-shadow:0 8px 22px rgba(0,0,0,0.24);`; }
  private inp(): string { return `width:100%;padding:14px 16px;border-radius:14px;border:2px solid rgba(49,80,51,0.25);font-size:18px;font-weight:800;color:#193620;outline:none;background:#fffdf2;`; }
  private mask(email: string): string { const [n, d] = email.split('@'); return n && d ? `${n.slice(0, 2)}***@${d}` : email; }
}
