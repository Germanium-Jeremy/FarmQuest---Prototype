...
app.use(cors({ origin(origin, callback) { if (!origin || allowedOrigins.includes(origin)) { callback(null, true); return; } callback(new Error('Origin not allowed')); } }));
...
app.get('/{*splat}', (req, res) => {
  if (req.path.startsWith('/api/') || req.path.startsWith('/vendor') || req.path.startsWith('/admin')) { res.status(404).json({ message: 'Not found.' }); return; }
  res.sendFile(resolve(distPath, 'index.html'));
});

app.use((err: Error, _req: Request, res: Response, _next: NextFunction) => { console.error(err); res.status(500).json({ message: 'Something went wrong.' }); });
...
function rateLimit(req: Request, res: Response, next: NextFunction): void {
  ...
  const key = req.ip ?? 'unknown'; const now = Date.now(); const windowMs = 60_000;
  const current = hits.get(key);
  if (!current || current.resetAt <= now) { hits.set(key, { count: 1, resetAt: now + windowMs }); next(); return; }
  current.count += 1;
  if (current.count > 60) { res.status(429).json({ message: 'Please wait before trying again.' }); return; }
  next();
}
